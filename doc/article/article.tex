...
\textbf{Double Convolution Block:} Each double convolution block applies two consecutive $3 \times 3$ convolutions with padding=1, followed by batch normalization and ReLU activation after each convolution. This preserves spatial dimensions while increasing feature depth.

\textbf{Upsampling Block:} The upsampling blocks use $2 \times 2$ transposed convolutions with stride=2 to double the spatial resolution, followed by batch normalization and ReLU activation.

\textbf{Skip Connections:} The architecture employs skip connections that concatenate feature maps from the encoder path with corresponding decoder features, doubling the channel dimension before processing through decoder blocks.

The network processes input images of size $256 \times 256$ through a symmetric encoder-decoder structure. The encoder progressively reduces spatial resolution from $256^2$ to $128^2$ to $64^2$ while increasing feature channels from 32 to 64 to 128. The decoder reverses this process, upsampling back to the original resolution while maintaining rich feature representations through skip connections. The final $1 \times 1$ convolution maps the 32-channel feature representation to 3 output classes for pixel-wise classification.
